        ...
        if (reduction != null) {
            if (reduction.size() == 3) {
                ...
                if (validOperators.indexOf(theOperator + " ") == -1) {
                    parser.raiseParserException(Py.formatMessage("error.invalid_operator", validOperators, theOperator));
                }
            } else {
    ...
    public Variable getValue() throws ParserException {
        ...
        if (theOperator.equals("==")) {
            if (bothValuesAreNumbers()) {
                ...
                theParser.raiseParserException(Py.formatMessage("error.type_mismatch"));
            }
