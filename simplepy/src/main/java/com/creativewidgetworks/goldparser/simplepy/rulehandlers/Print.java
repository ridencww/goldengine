...
@ProcessRule(rule = {
    ...
    "<Statement> ::= print <Expression> read Id"
})
    ...
    public Print(GOLDParser parser) {
        ...
        if (reduction != null) {
            if (reduction.size() == 2) {
                ...
                if (reduction.size() < 3) {
                    // print x form
                    parser.raiseParserException(Py.formatMessage("error.param_count", "2", String.valueOf(reduction.size())));
                }
                // print x read y form
                parser.raiseParserException(Py.formatMessage("error.param_count", "4", String.valueOf(reduction.size())));
            }
        } else {
